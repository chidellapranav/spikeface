...
int main() {
    ...
    for (int t = 0; t < GOLDEN_FC1_T; t++) {
        ...
        for (int i = 0; i < FC1_IN; i++)
            input_flat.write((pooled_t)GOLDEN_FC1_INPUT[t][i]);

        long active_taps = 0;
        fc1_layer_axi(input_flat, weights, spike_out, beta, threshold, active_taps);
    ...
    if (total_mismatches == 0) {
        printf("\nPASS: AXI-ready fc1 layer is bit-exact with the plain-array\n");
